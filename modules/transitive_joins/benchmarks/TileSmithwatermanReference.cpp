...
int main ( int argc, char* argv[] ) {
    ...
    hclib::launch(deps, 1, [=] {
        ...
        for (int i = n_tiles_height; i >= 1; i-- ) {
            for (int j = n_tiles_width; j >= 1; j-- ) {
                hclib::async([=] {
                    int index, ii, jj;
        ...
        int score = ((int *)(tile_matrix[n_tiles_height][n_tiles_width].bottom_row->get_future()->wait()))[tile_width-1];
        gettimeofday(&end,0);
