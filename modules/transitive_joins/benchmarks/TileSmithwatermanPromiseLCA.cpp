...
typedef struct {
    tj::TJPromise<int*>* bottom_row;
    tj::TJPromise<int*>* right_column;
    tj::TJPromise<int*>* bottom_right;
} Tile_t;
    ...
    hclib::launch(deps, 2, [=] {
        ...
        for ( i = 0; i < n_tiles_height+1; ++i ) {
            ...
            for ( j = 0; j < n_tiles_width+1; ++j ) {
                tile_matrix[i][j].bottom_row = tj::getNewTJPromise<int*>();
                tile_matrix[i][j].right_column = tj::getNewTJPromise<int*>();
                tile_matrix[i][j].bottom_right = tj::getNewTJPromise<int*>();
            }
        }
        ...
        fprintf(stdout, "Executing the algorithm\n");
        struct timeval begin,end;
        gettimeofday(&begin,0);

        for (int i = 1; i <= n_tiles_height; i++ ) {
            for (int j = 1; j <= n_tiles_width; j++ ) {
                std::vector<tj::TJPromiseBase*> promises;
                promises.push_back(tile_matrix[i][j].bottom_right);
                promises.push_back(tile_matrix[i][j].bottom_row);
                promises.push_back(tile_matrix[i][j].right_column);

                tj::async(promises, [=] {
                    int index, ii, jj;
                    int* above_tile_bottom_row = (int *)tile_matrix[i-1][j  ].bottom_row->wait();
                    int* left_tile_right_column = (int *)tile_matrix[  i][j-1].right_column->wait();
                    int* diagonal_tile_bottom_right = (int *)tile_matrix[i-1][j-1].bottom_right->wait();
        ...
        int score = ((int *)(tile_matrix[n_tiles_height][n_tiles_width].bottom_row->wait()))[tile_width-1];
        gettimeofday(&end,0);
